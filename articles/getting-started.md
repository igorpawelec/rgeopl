...
Every question in this package has the same shape: *what exists here,
and which of it do I want?* Asking is one step, downloading is another,
and nothing is fetched until you say so.
...
takes whatever you already have. A point and a radius:
...
The CRS was not given and did not need to be: those coordinates fall
inside Poland’s longitude/latitude window, and the PL-1992 window does
not overlap it. Coordinates outside both are refused rather than guessed
at.
...
Three indexes, one per kind of product. Nothing is downloaded; each
returns one row per tile, with the tile outline as geometry.
...
The elevation index mixes three products, and the counts per vintage say
a lot about what is actually possible:
...
Terrain models exist for ten years here. Surface models – the other half
of a canopy height model – exist for two.

## Which vintage actually covers the area

An index tells you tiles exist. It does not tell you whether they cover
your area, and a vintage that covers 80% of it cannot be mosaicked into
one surface.
...
`aoi_share` is the number to read. The same thing as a picture, one
panel per vintage:
...
![plot of chunk ortho-coverage](figures/ortho-coverage-1.png)

plot of chunk ortho-coverage

Small multiples rather than one map, deliberately: vintages overlap
almost completely, so a single overlaid map shows the newest year and
hides every other – the opposite of what you need in order to choose
between them.
...
Heights are not comparable across that boundary.
[`tile_download()`](https://igorpawelec.github.io/rgeopl/reference/tile_download.md)
warns when a selection mixes the two. It does not bite a canopy height
model – surface minus terrain within one year cancels the datum – but it
does bite any direct comparison of two terrain models.

**Resolution and density are different quantities.** The service reports
both through one text field, in different units: `"1.00 m"` for a grid,
`"10 p/m2"` for a point cloud. They are split here, because a filter
like `resolution <= 1` that silently also caught the sparsest point
clouds would be worse than useless.
...
The index is an ordinary `sf` data frame. Filter it however you like –
base R, `dplyr`,
[`sf::st_filter()`](https://r-spatial.github.io/sf/reference/st_join.html)
– and pass what survives to
[`tile_download()`](https://igorpawelec.github.io/rgeopl/reference/tile_download.md).
The class does not have to survive the pipeline;
[`tile_download()`](https://igorpawelec.github.io/rgeopl/reference/tile_download.md)
only needs the `URL` and `filename` columns.
...
Files land in the cache, so a tile already on disk is not fetched twice,
across sessions. `outdir` copies them somewhere convenient; it is an
export, not a move.
...
The service behind these indexes returns at most 1000 records per query
and sets a flag to say it truncated. Ask for a large area and you get a
thousand tiles and no error.

That is handled internally: the record count is read first, and when it
exceeds one page the results are assembled by object id instead. A 40 x
40 km area returns 19 766 index records in 20 requests, with the
assembled row count checked against the count the server promised. A
short result raises a warning rather than passing as complete.
...
The GUGiK services refuse connections from outside Poland; the BDL
services do not. Working on the elevation and orthophoto side from
abroad needs a VPN endpoint in Poland, and the error message says so
when a request fails against those hosts.

The forest side is covered in
[`vignette("forest-data")`](https://igorpawelec.github.io/rgeopl/articles/forest-data.md),
and the two are put together in
[`vignette("case-study")`](https://igorpawelec.github.io/rgeopl/articles/case-study.md).
