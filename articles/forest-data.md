...
The Forest Data Bank draws conservation areas on its maps and publishes
none of them as data: its whole public service is seven layers of
administrative units and subareas. The register belongs to GDOŚ, and
...
Six features from four registers. `code` is the Natura 2000 site code
and is empty elsewhere, because only Natura 2000 has one. Ask for one
register, or for both directives at once:
...
This is the question the two halves of this vignette meet on: the
subareas come from BDL, the protection from GDOŚ, and the answer is an
intersection.
...
Most of these overlap hardly anything – seven stands in the birds
directive site, two in the habitats one – while the landscape complexes
cover nearly three quarters of the area. That spread is the point: “is
this protected” has no single answer, and which register you mean
changes it by two orders of magnitude.
...
![plot of chunk warmia-protected](figures/warmia-protected-1.png)

plot of chunk warmia-protected

One thing to know before summing areas: a park and its buffer zone are
separate features under the same name. `ParkiNarodowe` returns 46 rows
for Poland’s 23 national parks, exactly 23 of them buffer zones, so
counting or measuring without looking counts the same ground twice.
